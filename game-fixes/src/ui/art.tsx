...
export function HeroArt({ classId, equip, className = "w-full h-full" }: {
  ...
  const item = (s: Slot) => equip?.[s];
  const legendary = Object.values(equip ?? {}).some(i => i && i.rarity === 4);
  const rare = Object.values(equip ?? {}).some(i => i && i.rarity >= 3);
  const isMage = classId === "mage";
  const robe = isMage ? "#3b6fd4" : "#2f8a4d";
  const robeD = isMage ? "#24448a" : "#1d5c33";
  const weaponItem = item("weapon");
  const helmItem = item("helm");
  const armorItem = item("armor");
  const glovesItem = item("gloves");
  const bootsItem = item("boots");
  const amuletItem = item("amulet");
  const ring1Item = item("ring1");
  const ring2Item = item("ring2");

  const isRanged = weaponItem?.base === "weapon" && weaponItem.name.toLowerCase().includes("лук");
  const weaponRarity = weaponItem?.rarity ?? 0;
  const weaponRarityForEffect = weaponItem?.rarity ?? 0;

  const equipColor = (rarity: number) => {
    switch (rarity) {
      case 5: return "#ff4d6d";
      case 4: return "#fbbf24";
      case 3: return "#c084fc";
  ...
  const staffGlow = isMage && weaponItem;

  return (
    <svg viewBox="0 0 100 130" className={`${className} hero-breathe`} aria-hidden>
      {legendary && <ellipse cx="50" cy="108" rx="34" ry="10" fill="#fbbf24" opacity="0.18" className="hero-glow-legendary" />}
      {rare && !legendary && <ellipse cx="50" cy="108" rx="30" ry="8" fill="#c084fc" opacity="0.12" />}
      <ellipse cx="50" cy="112" rx="30" ry="8" fill="#000" opacity="0.35" />
      <ellipse cx="50" cy="122" rx="25" ry="4" fill="#000" opacity="0.2" />

      {isMage ? (
        <>
          <g className="mage-robe-idle">
            <path d="M50 46 L76 114 Q50 124 24 114 Z" fill={robe} stroke={robeD} strokeWidth="3" />
            <path d="M30 80 L70 80 L72 90 Q50 95 28 90 Z" fill={robeD} opacity="0.5" />
          </g>
          <path d="M35 108 L38 118 L42 118 L40 108 Z" fill="#1a1a2e" />
          <path d="M60 108 L62 118 L66 118 L64 108 Z" fill="#1a1a2e" />
          {has("armor") && (
            <rect x="36" y="78" width="28" height="7" rx="3" fill={equipColor(armorItem?.rarity ?? 0)} stroke={equipColor(armorItem?.rarity ?? 0)} strokeWidth="1" opacity="0.9" />
          )}
          <g className={staffGlow ? "mage-staff-glow" : ""}>
            <path d="M80 34 L88 108" stroke="#7a5230" strokeWidth="5" strokeLinecap="round" />
            <circle cx="80" cy="30" r="9" fill={weaponRarity >= 3 ? equipColor(weaponRarity) : "#4cc3ff"} opacity="0.9" className="hero-attack-sword">
              {weaponRarity >= 4 && <animate attributeName="r" values="9;11;9" dur="1.5s" repeatCount="indefinite" />}
            </circle>
            <circle cx="80" cy="30" r="4" fill="#fff" opacity="0.95" />
            {weaponRarity >= 5 && <circle cx="80" cy="30" r="12" fill="none" stroke="#ff4d6d" strokeWidth="1" opacity="0.5">
              <animate attributeName="r" values="12;16;12" dur="2s" repeatCount="indefinite" />
              <animate attributeName="opacity" values="0.5;0;0.5" dur="2s" repeatCount="indefinite" />
            </circle>}
          </g>
          <circle cx="50" cy="36" r="14" fill="#f0c89a" stroke="#b98d5a" strokeWidth="2.5" className="hero-idle-head" />
          <path d="M32 34 Q50 -10 68 34 Q50 26 32 34 Z" fill={has("helm") ? equipColor(helmItem?.rarity ?? 0) : robe} stroke={robeD} strokeWidth="2.5" className="hero-layer-helm" />
          ...
          <path d="M30 92 L24 108" stroke={robe} strokeWidth="8" strokeLinecap="round" />
          <path d="M70 92 L76 108" stroke={robe} strokeWidth="8" strokeLinecap="round" />
          {has("gloves") && (
            <>
              <circle cx="24" cy="108" r="6" fill={equipColor(glovesItem?.rarity ?? 0)} stroke="#222" strokeWidth="1" />
              <circle cx="76" cy="108" r="6" fill={equipColor(glovesItem?.rarity ?? 0)} stroke="#222" strokeWidth="1" />
              <circle cx="24" cy="108" r="2" fill="#fff" opacity="0.5" />
              <circle cx="76" cy="108" r="2" fill="#fff" opacity="0.5" />
            </>
          )}
          {has("boots") && (
            <>
              <rect x="34" y="112" width="14" height="8" rx="3" fill={equipColor(bootsItem?.rarity ?? 0)} stroke="#222" strokeWidth="1" />
              <rect x="52" y="112" width="14" height="8" rx="3" fill={equipColor(bootsItem?.rarity ?? 0)} stroke="#222" strokeWidth="1" />
              <line x1="36" y1="114" x2="40" y2="118" stroke="#fff" opacity="0.3" />
              <line x1="54" y1="114" x2="58" y2="118" stroke="#fff" opacity="0.3" />
            </>
          )}
          {has("amulet") && <circle cx="50" cy="60" r="5" fill={equipColor(amuletItem?.rarity ?? 0)} stroke="#fff" strokeWidth="1" className="hero-layer-amulet">
            <animate attributeName="opacity" values="0.7;1;0.7" dur="2s" repeatCount="indefinite" />
          </circle>}
          {has("ring1") && <circle cx="28" cy="100" r="4" fill={equipColor(ring1Item?.rarity ?? 0)} stroke="#fff" strokeWidth="0.8" className="hero-layer-ring">
            <animate attributeName="opacity" values="0.6;1;0.6" dur="2.5s" repeatCount="indefinite" />
          </circle>}
          {has("ring2") && <circle cx="72" cy="100" r="4" fill={equipColor(ring2Item?.rarity ?? 0)} stroke="#fff" strokeWidth="0.8" className="hero-layer-ring">
            <animate attributeName="opacity" values="0.6;1;0.6" dur="3s" repeatCount="indefinite" />
          </circle>}
          {weaponRarity >= 4 && (
            <circle cx="50" cy="65" r="25" fill="none" stroke={equipColor(weaponRarity)} strokeWidth="1" opacity="0.3">
        ...
        <>
          ...
          <path d="M60 108 L62 118 L66 118 L64 108 Z" fill="#1a1a2e" />
          {has("armor") && (
            <rect x="36" y="76" width="28" height="8" rx="3" fill={equipColor(armorItem?.rarity ?? 0)} stroke={equipColor(armorItem?.rarity ?? 0)} strokeWidth="1" opacity="0.9" />
          )}
          <path d="M62 50 L74 34 L79 38 L67 54 Z" fill="#3a2a20" stroke="#2a1a10" strokeWidth="2" />
          <path d="M71 33 l6-6 M74 37 l6-6" stroke="#c9d4de" strokeWidth="2" />
          {has("weapon") ? (
            <g className={isRanged ? "hero-attack-bow" : "hero-attack-sword"}>
              <path d={isRanged ? "M22 48 Q6 80 22 112" : "M22 48 Q6 80 22 112"} stroke={equipColor(weaponRarityForEffect)} strokeWidth="5" fill="none" strokeLinecap="round" />
              <path d="M22 48 L22 112" stroke={equipColor(weaponRarityForEffect)} strokeWidth="1.5" />
              {weaponRarityForEffect >= 3 && (
                <path d={isRanged ? "M18 45 Q4 78 18 110" : "M18 45 Q4 78 18 110"} stroke="#fff" strokeWidth="1" opacity="0.5" />
              )}
              {weaponRarityForEffect >= 5 && (
                <path d={isRanged ? "M20 46 Q8 79 20 111" : "M20 46 Q8 79 20 111"} stroke={equipColor(weaponRarityForEffect)} strokeWidth="1" opacity="0.6">
            ...
            <>
              <path d="M22 48 Q10 80 22 112" stroke="#8a5f22" strokeWidth="5" fill="none" strokeLinecap="round" />
              <path d="M22 48 L22 112" stroke="#c9d4de" strokeWidth="1.5" />
            </>
          )}
          <circle cx="50" cy="36" r="14" fill="#f0c89a" stroke="#b98d5a" strokeWidth="2.5" className="hero-idle-head" />
          <path d="M36 32 Q38 18 50 18 Q62 18 64 32 Q58 24 50 26 Q42 24 36 32 Z" fill={has("helm") ? equipColor(helmItem?.rarity ?? 0) : "#5a3a22"} stroke="#3a2510" strokeWidth="2" className="hero-layer-helm" />
          <circle cx="45" cy="38" r="2" fill="#22252c" /><circle cx="55" cy="38" r="2" fill="#22252c" />
          <path d="M46 45 Q50 48 54 45" stroke="#22252c" strokeWidth="2" fill="none" strokeLinecap="round" />
          <path d="M32 92 L26 108" stroke={robe} strokeWidth="8" strokeLinecap="round" />
          <path d="M68 92 L74 108" stroke={robe} strokeWidth="8" strokeLinecap="round" />
          {has("gloves") && (
            <>
              <circle cx="26" cy="108" r="6" fill={equipColor(glovesItem?.rarity ?? 0)} stroke="#222" strokeWidth="1" />
              <circle cx="74" cy="108" r="6" fill={equipColor(glovesItem?.rarity ?? 0)} stroke="#222" strokeWidth="1" />
              <circle cx="26" cy="108" r="2" fill="#fff" opacity="0.5" />
              <circle cx="74" cy="108" r="2" fill="#fff" opacity="0.5" />
            </>
          )}
          {has("boots") && (
            <>
          ...
          <path d="M80 50 l6-6 M82 52 l4-4" stroke="#c9d4de" strokeWidth="2" opacity="0.6" />
          <rect x="78" y="46" width="8" height="3" rx="1" fill="#8a5f22" opacity="0.5" />
          {weaponRarityForEffect >= 4 && (
            <path d="M22 60 L10 55 M22 60 L8 65" stroke={equipColor(weaponRarityForEffect)} strokeWidth="2" opacity="0.5">
      ...
      {rare && !legendary && (
        <circle cx="50" cy="65" r="30" fill="none" stroke={equipColor(3)} strokeWidth="0.5" opacity="0.2">
