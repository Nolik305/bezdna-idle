...
// Серверный симулятор (tsx loader нужен для импорта .ts файлов)
import { settleState, validateClientState, isProgressSafe, getOfflineIncome } from "./state-simulator.ts";
...
/** Множество допустимых ключей оборудования для валидации сейва. */
const VALID_SLOTS = new Set(["weapon", "helm", "amulet", "armor", "gloves", "boots", "ring1", "ring2"]);
const VALID_STAT_KEYS = new Set(["dmg", "dmgPct", "hp", "hpPct", "armor", "crit", "critDmg", "as", "goldPct", "xpPct", "luck", "regen"]);

/** Проверяет, что значение — конечное неотрицательное число. */
function finiteNonNegative(v: unknown): boolean {
  return typeof v === "number" && Number.isFinite(v) && v >= 0;
}

/** Проверяет, что значение — целое число в допустимом диапазоне. */
function safeInteger(v: unknown, min = 0, max = 9999): boolean {
  return Number.isInteger(v as number) && (v as number) >= min && (v as number) <= max;
}

/** Проверяет, что строка — допустимый класс. */
function validClassId(v: unknown): boolean {
  return v === "mage" || v === "archer";
}

/** Ленивая валидация произвольного объекта-рекорда (Record<string, unknown>).
 *  Не бросает ошибку при отсутствии полей — просто пропускает. */
function isValidRecord(v: unknown): boolean {
  return v === null || v === undefined || (typeof v === "object" && !Array.isArray(v));
}

/** Полная валидация состояния игры с учётом всех новых систем. */
function validGameState(game: unknown): boolean {
  ...
  // Базовые поля героя
  if (!validClassId(h["classId"])) return false;
  if (!safeInteger(h["level"], 1, 9999)) return false;
  if (!finiteNonNegative(h["gold"])) return false;
  if (!finiteNonNegative(h["gems"])) return false;
  if (!finiteNonNegative(h["potions"])) return false;

  // Инвентарь
  if (!Array.isArray(g["inv"]) || (g["inv"] as unknown[]).length > 40) return false;

  // Базовые подсистемы (обязательные)
  for (const key of ["battle", "daily", "activity", "totals", "equip", "skills", "passives", "buffs", "toasts"]) {
    if (!g[key]) return false;
  }
  if (!Array.isArray(g["buffs"])) return false;
  if (!isValidRecord(g["equip"])) return false;
  if (!isValidRecord(g["skills"])) return false;
  if (!isValidRecord(g["passives"])) return false;

  // Проверка equip: ключи — допустимые слоты
  const equip = g["equip"] as Record<string, unknown> | null;
  if (equip) {
  ...
  // Новые системы — валидируем если присутствуют (лениво)
  if (g["prestige"] != null && typeof g["prestige"] !== "object") return false;
  if (g["bestiary"] != null && typeof g["bestiary"] !== "object") return false;
  if (g["guildBoss"] != null && typeof g["guildBoss"] !== "object") return false;
  if (g["battlePass"] != null && typeof g["battlePass"] !== "object") return false;
  if (g["pet"] != null && typeof g["pet"] !== "object") return false;
  if (g["tournament"] != null && typeof g["tournament"] !== "object") return false;
  if (g["runes"] != null && typeof g["runes"] !== "object") return false;
  if (g["base"] != null && typeof g["base"] !== "object") return false;
  if (g["social"] != null && typeof g["social"] !== "object") return false;
  if (g["afkRewards"] != null && typeof g["afkRewards"] !== "object") return false;

  // Duel — если присутствует, проверяем структуру
  const duel = g["duel"] as Record<string, unknown> | null;
  if (duel && typeof duel === "object") {
    if (!safeInteger(duel["mmr"], 0)) return false;
    if (!safeInteger(duel["tokens"], 0)) return false;
    if (!safeInteger(duel["wins"], 0)) return false;
    if (!safeInteger(duel["losses"], 0)) return false;
  }

  // Run — если присутствует, проверяем
  const run = g["run"] as Record<string, unknown> | null;
  if (run && typeof run === "object") {
    if (!safeInteger(run["depth"], 0)) return false;
    if (!safeInteger(run["wave"], 0)) return false;
    if (!finiteNonNegative(run["goldEarned"])) return false;
  }

  // Party — если присутствует
  const party = g["party"];
  if (party != null && typeof party !== "object") return false;

  // Мета-поля
  if (!isValidRecord(g["meta"])) return false;
  if (!isValidRecord(g["resources"])) return false;
  if (!isValidRecord(g["autoSellRarities"])) return false;
  if (!isValidRecord(g["slotLevel"])) return false;
  if (!Array.isArray(g["bossDone"])) return false;
  if (!safeInteger(g["shards"], 0, Number.MAX_SAFE_INTEGER)) return false;
  if (g["godstone"] !== null && !safeInteger(g["godstone"], 0, Infinity)) return false;
  if (!safeInteger(g["portalDepth"], 0)) return false;
  if (!isValidRecord(g["atlas"])) return false;
  if (!finiteNonNegative(g["pathXp"] as unknown)) return false;
  if (!isValidRecord(g["seasonal"])) return false;
  if (!isValidRecord(g["shopBuys"])) return false;

  // Totals — обязательные поля
  const totals = g["totals"] as Record<string, unknown> | null;
  if (totals) {
    for (const k of ["kills", "bosses", "crits", "goldEarned", "dmgDealt", "items", "deaths", "casts"]) {
      if (!safeInteger(totals[k], 0, Number.MAX_SAFE_INTEGER)) return false;
    }
  }

  // Hero — дополнительные поля
  if (!finiteNonNegative(h["xp"])) return false;
  if (!finiteNonNegative(h["skillPoints"])) return false;

  return true;
}

function gameStateValidationDetail(game: unknown): string {
  if (!game || typeof game !== "object") return "state is not an object";
  const g = game as Record<string, any>;
  if (g.v !== 1) return `v=${String(g.v)}`;
  const h = g.hero;
  if (!h || typeof h !== "object") return "hero missing";
  if (!validClassId(h.classId)) return `hero.classId=${String(h.classId)}`;
  if (!safeInteger(h.level, 1, 9999)) return `hero.level=${String(h.level)}`;
  if (!finiteNonNegative(h.gold)) return `hero.gold=${String(h.gold)}`;
  if (!finiteNonNegative(h.gems)) return `hero.gems=${String(h.gems)}`;
  if (!finiteNonNegative(h.potions)) return `hero.potions=${String(h.potions)}`;
  if (!Array.isArray(g.inv) || g.inv.length > 40) return "inv missing or too large";
  for (const key of ["battle", "daily", "activity", "totals", "equip", "skills", "passives", "buffs", "toasts"]) {
    if (!g[key]) return `${key} missing`;
  }
  if (!Array.isArray(g.buffs)) return "buffs is not an array";
  if (!isValidRecord(g.equip)) return "equip is not an object";
  if (!isValidRecord(g.skills)) return "skills is not an object";
  if (!isValidRecord(g.passives)) return "passives is not an object";
  const equip = g.equip as Record<string, unknown>;
  for (const slot of Object.keys(equip)) if (!VALID_SLOTS.has(slot)) return `equip.${slot} is invalid`;
  for (const key of ["prestige", "bestiary", "guildBoss", "battlePass", "pet", "tournament", "runes", "base", "social", "afkRewards"]) {
    if (g[key] != null && typeof g[key] !== "object") return `${key} is not an object`;
  }
  if (g.duel && typeof g.duel === "object") {
    for (const key of ["mmr", "tokens", "wins", "losses"]) if (!safeInteger(g.duel[key], 0)) return `duel.${key}=${String(g.duel[key])}`;
  }
  if (g.run && typeof g.run === "object") {
    if (!safeInteger(g.run.depth, 0)) return `run.depth=${String(g.run.depth)}`;
    if (!safeInteger(g.run.wave, 0)) return `run.wave=${String(g.run.wave)}`;
    if (!finiteNonNegative(g.run.goldEarned)) return `run.goldEarned=${String(g.run.goldEarned)}`;
  }
  if (g.party != null && typeof g.party !== "object") return "party is not an object";
  for (const key of ["meta", "resources", "autoSellRarities", "slotLevel", "atlas", "seasonal", "shopBuys"]) {
    if (!isValidRecord(g[key])) return `${key} is not an object`;
  }
  if (!Array.isArray(g.bossDone)) return "bossDone is not an array";
  if (!safeInteger(g.shards, 0, Number.MAX_SAFE_INTEGER)) return `shards=${String(g.shards)}`;
  if (g.godstone !== null && !safeInteger(g.godstone, 0, Infinity)) return `godstone=${String(g.godstone)}`;
  if (!safeInteger(g.portalDepth, 0)) return `portalDepth=${String(g.portalDepth)}`;
  if (!finiteNonNegative(g.pathXp)) return `pathXp=${String(g.pathXp)}`;
  const totals = g.totals as Record<string, unknown>;
  for (const key of ["kills", "bosses", "crits", "goldEarned", "dmgDealt", "items", "deaths", "casts"]) {
    if (!safeInteger(totals[key], 0, Number.MAX_SAFE_INTEGER)) return `totals.${key}=${String(totals[key])}`;
  }
  if (!finiteNonNegative(h.xp)) return `hero.xp=${String(h.xp)}`;
  if (!finiteNonNegative(h.skillPoints)) return `hero.skillPoints=${String(h.skillPoints)}`;
...
// Нормализует строку: Unicode NFD → снимает диакритику, приводит к lowerCase
// Это позволяет обнаруживать "хулё" → "хуё", "хул€" → "хуё" и т.д.
function normalizeStr(s: string): string {
  return s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
}

// Базовый список (в normalizeStr-форме для сравнения).
// Дополнительная нормализация через NFD покрывает варианты с диакритикой.
const BAD_WORDS_RAW = ["хуй","хуя","хуё","пизд","бля","бляд","ебал","ебать","ебан","залуп","мудак","мудач","гандон","пидор","петух","шлюх","сука","сучка","охуе","нахуй","похуй","гнид","тварь","ублюд","долбоеб","долбоёб","срать","говно","дерьмо","мразь","козёл"];
const BAD_WORDS_NORMALIZED = BAD_WORDS_RAW.map(w => normalizeStr(w));

function filterText(raw) {
  let t = String(raw || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[<>]/g, "").trim().slice(0, CHAT_MAX_LEN);
  if (!t) return "";
  const tNorm = normalizeStr(t);
  for (const w of BAD_WORDS_NORMALIZED) {
    const re = new RegExp(w.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi");
    if (re.test(tNorm)) {
      // Заменяем каждое совпадение в оригинальной строке
      const wordRe = new RegExp(w.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi");
      t = t.replace(wordRe, "***");
    }
  }
  return t.trim();
  ...
  try {
    ...
    if (req.method === "PUT" && req.url === "/api/state") {
      ...
      if (!validGameState(game) || JSON.stringify(game).length > 512 * 1024) {
        const detail = JSON.stringify(game).length > 512 * 1024 ? "state too large" : gameStateValidationDetail(game);
        console.warn(`[state] invalid_game_state user=${id}: ${detail}`);
        return json(res, 400, { error: "invalid_game_state", detail });
      }
    ...
    /* ---- Settle эндпоинт (пакетная обработка) ---- */
    if (req.method === "POST" && req.url === "/api/settle") {
      const body = await readBody(req);
      const { lastTick, clientState, baseRevision } = body || {};
      if (!Number.isInteger(lastTick) || lastTick < 0) {
        return json(res, 400, { error: "invalid_lastTick" });
      }
      if (!clientState) {
        return json(res, 400, { error: "missing_client_state" });
      }
      // Валидация входного состояния
      const validationError = validateClientState(clientState);
      if (validationError) {
        return json(res, 400, { error: validationError });
      }
      // Получаем сохранённое состояние из БД
      const currentResult = await pool.query(
        "SELECT state, state_revision FROM players WHERE vk_user_id = $1", [id]
      );
      if (!currentResult.rowCount) {
        return json(res, 404, { error: "player_not_found" });
      }
      const savedState = currentResult.rows[0].state;
      const savedRevision = Number(currentResult.rows[0].state_revision) || 0;
      // Проверка прогресса
      if (!isProgressSafe(clientState, savedState)) {
        return json(res, 400, { error: "invalid_progress_delta" });
      }
      // Серверная симуляция
      const now = Date.now();
      const settleResult = settleState(savedState, clientState, lastTick, now);
      if (settleResult.rejected) {
        return json(res, 400, { error: "simulation_rejected", corrections: settleResult.corrections });
      }
      // Обновляем состояние в БД
      const updated = await pool.query(`
        UPDATE players
        SET state = $2::jsonb, state_revision = state_revision + 1, updated_at = now()
        WHERE vk_user_id = $1 AND state_revision = $3
        RETURNING state_revision
      `, [id, JSON.stringify(settleResult.state), savedRevision]);
      if (!updated.rowCount) {
        return json(res, 409, { error: "state_revision_conflict", revision: savedRevision });
      }
      const newRevision = Number(updated.rows[0].state_revision);
      if (settleResult.corrections.length > 0) {
        console.log(`[settle] user=${id} corrections:`, settleResult.corrections);
      }
      const offlineSeconds = (now - savedState.lastSeen) / 1000;
      const offlineIncome = getOfflineIncome(settleResult.state, Math.max(0, offlineSeconds));
      return json(res, 200, {
        state: settleResult.state,
        revision: newRevision,
        serverTime: settleResult.serverTime,
        corrections: settleResult.corrections.length > 0 ? settleResult.corrections : undefined,
        offlineIncome,
      });
    }
...
// Периодическая чистка throttles (memory leak fix)
setInterval(() => {
  const now = Date.now();
  for (const [key] of rateBuckets) {
    const bucket = rateBuckets.get(key);
    if (now - bucket.startedAt > 120_000) rateBuckets.delete(key);
  }
  for (const [key] of chatThrottle) {
    const rec = chatThrottle.get(key);
    if (now - rec.hourStart > 7200_000) chatThrottle.delete(key);
  }
}, 3600_000);
