...
export function getSaveKey(vkUserId?: number): string {
...
export function getStats(s: GameState): Stats {
  ...
  return {
    dmg, dps, as, crit: Math.min(85, crit), critDmg, maxHp, armor, mit, goldPct, xpPct, dmgPct,
  ...
  return SKILLS
    ...
    .reduce((total, def) => {
      ...
      const direct = stats.dmg * def.mult(level) * def.hits(level) * 0.6 * (1 + stats.extraSkillDamagePct / 100) / cooldown;
        const dot = def.dotPct ? stats.dmg * def.dotPct(level) * 5 * (1 + stats.extraSkillDamagePct / 100) / cooldown : 0;
...
export function reducer(s: GameState, a: Action): GameState {
  switch (a.type) {
    ...
    case "LEVEL_PASSIVE": {
      ...
      if (!def) return s;
      if (def.classId && def.classId !== s.hero.classId) return s;
    ...
    case "RESET": {
      try { localStorage.removeItem(getSaveKey()); } catch { /* noop */ }
      const fresh = newGame();
...
/* =============== persistence =============== */
export function saveGame(s: GameState, vkUserId?: number) {
  try {
...
export function migrateState(s: GameState): GameState {
  ...
  s.bestiary = {
    ...ref.bestiary,
    ...(s.bestiary || {}),
    entries: { ...ref.bestiary.entries, ...(s.bestiary?.entries || {}) },
    collectionBonus: { ...ref.bestiary.collectionBonus, ...(s.bestiary?.collectionBonus || {}) },
    bonuses: [...(s.bestiary?.bonuses || [])],
    maxKills: { ...ref.bestiary.maxKills, ...(s.bestiary?.maxKills || {}) },
  ...
  s.social = {
    ...
    giftsReceived: [...(s.social?.giftsReceived || [])],
    giftsSent: [...(s.social?.giftsSent || [])],
    referred: [...(s.social?.referred || [])],
    friendLeaderboard: [...(s.social?.friendLeaderboard || [])],
  };
...
export function loadGame(vkUserId?: number): GameState {
  let s: GameState;
  try {
    const raw = localStorage.getItem(getSaveKey(vkUserId));
