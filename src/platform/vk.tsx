...
export function VkProvider({ children }: { children: ReactNode }) {
  ...
  const performSave = async (state: GameState): Promise<boolean> => {
    ...
    const payload = await response?.json().catch(() => null) as { revision?: number; error?: string; detail?: string } | null;
    if (response?.ok) {
      ...
      const reason = response?.status === 409
        ? `конфликт версии (${payload?.error || "state_revision_conflict"}) — загрузите игру заново`
        : response
        ? `HTTP ${response.status}: ${payload?.error || "ошибка сервера"}${payload?.detail ? ` (${payload.detail})` : ""}`
        : "нет соединения";
