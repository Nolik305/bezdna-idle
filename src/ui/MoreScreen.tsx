...
function OptSeg() {
  const { s, d } = useGame();
  const { inVk, launchParams, initStage, user, friends, serverMmr, serverProfile, leaderboard, cloudStatus, cloudDetail, diag, saveGameState } = useVk();
  ...
  return (
    <div className="panel p-3 flex flex-col gap-3">
      ...
      <button onClick={async () => { if (inVk) await saveGameState(s); else saveGame(s); setSaved(true); setTimeout(() => setSaved(false), 1500); }}
